...
// dataById solo contiene obras COMPLETAS (nuevas, o cargadas para actualizar).
// Las filas livianas de loadCatalogLight() nunca deben llegar a un upsert.
...
export async function runManhwaWebScraper() {
  ...
  try {
    ...
    // Catálogo liviano: sin el JSON pesado de capitulos/categorias/autores.
    const data = await loadCatalogLight();
...
for (const remote of catalog) {
      ...
      if (!local && nameKey) {
        ...
        if (local) {
          ...
          if (dataById.has(String(local.id))) {
            // obra nueva de esta misma corrida: aún no está en la DB, va por upsert
            dirty.add(String(local.id));
          } else {
            relinks.set(String(local.id), {
              id: String(local.id),
              remoteId,
              nameKey: local.nameKey || nameKey,
            });
          }
          relinkedCount++;
        }
      ...
      const missingChapters = !local.chapterCount;
    ...
    // Re-vinculaciones: solo remoteId/nameKey, sin reescribir la fila completa.
    if (relinks.size > 0) await relinkManhwaBatch([...relinks.values()]);
    ...
    async function worker() {
      while (true) {
        ...
        try {
          if (job.type === "UPDATE") {
            // Recién ahora se trae la fila completa (una sola obra).
            const full = await findManhwaById(manga.id);
            if (!full) throw new Error("obra no encontrada en la DB");
            full.remoteId = manga.remoteId || full.remoteId;
            if (!full.nameKey && manga.nameKey) full.nameKey = manga.nameKey;
            manga = full;
            dataById.set(String(manga.id), manga);
          }
          ...
          failed++;
          if (job.type === "NEW") dirty.add(String(job.manga.id));
        }
