...
import { loadCatalogLight, findManhwaById, upsertManhwaBatch } from "../catalog.js";
...
export async function runOlympusScraper() {
  ...
  try {
    ...
    const catalog = await loadCatalogLight();
    ...
    for (const remote of remoteList) {
      ...
      if (!local) {
        ...
        const localCap = local.chapterCount || 0;
        const needsUpdate =
          remoteCap > localCap ||
          !local.hasSinopsis ||
          localCap === 0;
    ...
    async function worker() {
      while (true) {
        ...
        try {
          if (job.type === "UPDATE") {
            // Recién ahora se trae la fila completa (una sola obra) y se
            // le aplican los cambios livianos detectados arriba.
            const full = await findManhwaById(manga.id);
            if (!full) throw new Error("obra no encontrada en la DB");
            full.remoteId = manga.remoteId;
            full.plataforma = manga.plataforma;
            if (manga.nombre) {
              full.nombre = manga.nombre;
              full.nameKey = manga.nameKey;
            }
            if (manga.imagen) full.imagen = manga.imagen;
            manga = full;
          }
          ...
          failed++;
          console.warn(`[olympus] falló ${job.manga.id}: ${err.message}`);
          if (job.type === "NEW") dirty.push(job.manga);
        }
        await sleep(DELAY);
      }
    }
