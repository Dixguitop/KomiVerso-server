...
import { loadCatalog, findManhwaById } from "../catalog.js";
    ...
    try {
      ...
      if (isOlympus) {
        ...
        if (!slug || !olympusChapterId) {
          throw new Error(
            "Capítulo Olympus sin slug o sin id de capítulo guardado"
          );
        }
        const images = await fetchOlympusPages(slug, olympusChapterId);
        if (!images.length) {
          throw new Error("Olympus no devolvió imágenes para este capítulo");
        }
